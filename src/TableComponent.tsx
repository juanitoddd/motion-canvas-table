...
export class Table extends Shape {
    ...
    this.add(
        ...
        ref={this.container}
			...
			>
              ...
              justifyContent={props.style?.headerStyle?.justifyContent ||'center'} 
              padding={props.style?.headerStyle?.padding || 10}
              width={'100%'}              
            >
              {['string', 'number'].includes(typeof header.label) ? 
							  <Txt fill={props.style?.headerStyle?.color ||'#fff'}>{header.label.toString()}</Txt> : header.label
              }
						</Rect>            
            <Node ref={makeRef(this.colNodes, i)}>
              {this.cols[i].map((cell: RowCell) => (
                <Rect 
                  ...
                  justifyContent={props.style?.rowStyle ?.justifyContent ||'center'} 
                  padding={props.style?.rowStyle?.padding || 10}
                >
                  {cell ? ['string', 'number'].includes(typeof cell.label) ? 
							      <Txt fill={props.style?.headerStyle?.color ||'#fff'}>{cell.label.toString()}</Txt> : cell.label
                  : null}
                </Rect>
              ))}
            </Node>            
  ...
  public *addRow(row: RowCell[], duration: number) {    
    const cellRect: Rect[] = []
    const cols: RowCell[] = this.props.data.headers.map((header: HeaderCell) => row.find((cell: RowCell) => cell.group === header.id))    
    cols.map((cell: RowCell, j) => (
      this.colNodes[j].add(
        <Rect 
          ...
          justifyContent={this.props.style?.rowStyle ?.justifyContent ||'center'} 
          height={'100%'}
          padding={0}          
          opacity={0}               
        >
          {cell ? ['string', 'number'].includes(typeof cell.label) ? 
            <Txt fill={this.props.style?.headerStyle?.color ||'#fff'}>{cell.label.toString()}</Txt>:cell.label
            : null
          }
        </Rect>        
      )
    ...
    yield* all(
      ...
      ...cellRect.map((_rowRect: Rect) => _rowRect.padding(this.props.style?.rowStyle?.padding || 10, duration)),      
    )      
  }
